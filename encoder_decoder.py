...
class Decoder(nn.Module):
    def __init__(
        self, vocab_size, embed_size, hidden_size, num_layers=1, use_attention=True
    ):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embed_size, padding_idx=PAD_TOKEN)
        self.use_attention = use_attention
        self.gru = nn.GRU(
            embed_size + (hidden_size if use_attention else 0),
            hidden_size,
    ...
    def forward(self, src, src_lengths, tgt, teacher_forcing_ratio=0.5):
        ...
        for t in range(1, tgt_len):
            output, hidden = self.decoder(
                input_step, hidden, encoder_outputs, mask=mask
            )
            outputs[:, t] = output
...
def main():
    ...
    decoder = Decoder(
        VOCAB_SIZE, EMBED_SIZE, HIDDEN_SIZE, NUM_LAYERS, use_attention=True
    ).to(DEVICE)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            f"Epoch {epoch:02d} | Loss: {loss:.4f} | Val token accuracy: {acc*100:.2f}%"
        )
    model.eval()
    with torch.no_grad():
        for _ in range(8):
            ...
            outputs = model(
                src_b, src_l, tgt.unsqueeze(0).to(DEVICE), teacher_forcing_ratio=0.0
            )
