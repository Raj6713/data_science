...
Node = namedtuple("Node", ["features", "threshold", "left", "right", "value", "is_leaf"])

class DecisionTreeTorch:
    def __init__(self, max_depth=10, min_samples_split=2, criterion="gini", device=None):
    ...
    def fit(self, X, y):
        ...
        mapped = torch.tensor([self.class_map[int(val)] for val in y], device=self.device, dtype=torch.long)
    ...
    def _build_tree(self, X,y, depth):
        ...
        if (depth>=self.max_depth) or (n_samples < self.min_samples_split) or (num_samples_per_class.max().item()==n_samples):
            leaf = Node(features=None, threshold=None, left=None, right=None, value=predicted_class, is_leaf=True)
        ...
        node = Node(features=int(feature), threshold=float(threshold), left=left_node, right=right_node, value=None, is_leaf=False)
    ...
    def _best_split(self, X, y):
        ...
        for feature in range(n_features):
            ...
            unique_vals, inv_idx, counts = torch.unique_consecutive(sorted_vals, return_inverse=True, return_counts=True)
            if unique_vals.size(0)==1:
                continue
            prefix_counts = torch.zeros((self.n_classes_,n_samples), device=self.device, dtype=torch.int32)
            ...
            for pos in candidate_pos:
                ...
                if n_left < self.min_samples_split or n_right < self.min_samples_split:
                    continue
                right_counts = (prefix_counts[:,-1]-prefix_counts[:,pos]).to(torch.float32)
    ...
    def print_tree(self, node=None,depth=0):
        ...
        if node.is_leaf:
            ...
            print(f"{indent}Leaf: class: {label}, probs={node.value_proba.cpu().numpy()}")
        else:
